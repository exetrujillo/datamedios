...
extraer_noticias_max_res_ciper <- function(search_query = NULL, max_results = NULL) {
  ...
  # Filtrar columnas para mantener consistencia con crear_df_vacio()
  cols_standard <- names(crear_df_vacio())
  
  # Asegurar que todas las columnas estandar existan
  for (col in cols_standard) {
    if (!col %in% names(output_df)) {
      if (col == "temas") {
        output_df[[col]] <- vector("list", nrow(output_df))
      } else {
        output_df[[col]] <- NA_character_
      }
    }
  }
